;(function ($){
    ...
    var calendar = {
            ...
            return $('<ul class="calendar_head">' + arr.join('') + '</ul>').on('click','li span',function (){
                if ($(this).parent().prop('class') === 'prev'){
                    if ($(this).prop('class') === 'year'){
                        ...
                        var operation = myprivate.operateMonth(year,month - 1);
        ...
        getMonthDom: function (obj,opts){
            ...
            var days = myprivate.getDaysOfMonth(date); // 选择月的天数
            var beginWeek = myprivate.getFirstWeek(date); // 选择月1号周
            var endWeek = myprivate.getLastWeek(date); // 选择月最后一天周
            var monthDom = new Array(); // 用于构建DOM的数组

            // 构建上月日期
            if (opts.showMonth){
                var operation = myprivate.operateMonth(year,month - 1);
                var prevDays = myprivate.getDaysOfMonth(new Date(operation[0],operation[1],1)); // 获取上个月的天数
            ...
            for (var k = 0; k < days; k += 1){
                ...
                if (opts.settingdata.length){
                    ...
                    var filterArr = opts.settingdata
                        ...
                        _pd = filterArr.length ? '<span>' + (filterArr[0].currPrice ? '￥' + filterArr[0].currPrice : '无价格') + '</span><strong>' + (filterArr[0].roomSet&&filterArr[0].roomSet==='F'?'不限量':(filterArr[0].leftSum || '无库存')) + '</strong>' : '';
            ...
            return $('<ul class="calendar_body">' + monthDom.join('')+ '</ul>').on('click','li:not(.item-empty)',function (){
                ...
                var numTag = myprivate.numTag,memoryTag = myprivate.memoryTag;
        ...
        init: function (obj,opts){
            ...
            for (var i = 0; i < opts.multipleMonth; i += 1){
                ...
                var operation = myprivate.operateMonth(year,month + (i === 0 ? 0 : 1));
