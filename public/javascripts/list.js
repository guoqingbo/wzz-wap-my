$(function () {
    ...
    var dropBox = $(".drop-box"),
        ...
        localUrl = location.pathname +'s'+ window.location.search,
    ...
    function hotelInit() {
        ...
        filterObj.searchName = $('#searchName').val();
        unLockDropload();
        filterFn(dropload, 1);
    }
    ...
    function filterFn(dropload, startPage) {
        // alert(111);
        // localUrl.date= new Date().getTime();
        dt=false;
        // $('.drop-box ul').html('');
        $.ajax({
            type: 'POST',
            ...
            success: function (data) {
                if (data !== 'error' && data[0].flag !== 'error') {
                    ...
                    //filterObj.currPage = +(results.currPage) + 1;
                    filterObj.currPage += 1;
    ...
    // 订单列表切换类型history replace
    $('#myOrderTab a').click(function (e) {
        ...
        if (module === 'order') {
            ...
            switch (module) {
                ...
                case 'strategy':
                    ...
                    if (list[len].url || list[len].link){
                        url=list[len].url || list[len].link
                    }
                    break;
                case 'car':
